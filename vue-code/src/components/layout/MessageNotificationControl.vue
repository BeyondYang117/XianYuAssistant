...
const togglePanel = () => {
  ...
  // 右对齐按钮并夹在视口内，避免遮挡侧边栏内容
  const left = Math.min(Math.max(rect.right - width, 12), window.innerWidth - width - 12)
  panelStyle.value = { top: `${rect.bottom + 8}px`, left: `${Math.max(left, 12)}px` }
...
<template>
  <div class="message-notification-control">
    <button ref="buttonRef" class="message-notification-button" :class="{ 'has-unread': unreadCount }" title="消息通知" @click="togglePanel">
      <IconBell />
      <span v-if="unreadCount" class="message-notification-count">{{ unreadCount > 99 ? '99+' : unreadCount }}</span>
    </button>
    <Teleport to="body">
      <div v-if="panelOpen" class="message-notification-backdrop" @click="panelOpen = false"></div>
      <div v-if="panelOpen" class="message-notification-panel" :style="panelStyle">
...
.message-notification-backdrop { position: fixed; inset: 0; z-index: 1199; }
.message-notification-panel { position: fixed; width: 260px; padding: 14px; border: 1px solid var(--glass-border); border-radius: 12px; background: #fff; -webkit-backdrop-filter: var(--glass-blur); backdrop-filter: var(--glass-blur); box-shadow: var(--glass-shadow-float); color: var(--apple-text); z-index: 1200; box-sizing: border-box; }
...
@media (max-width: 767px) { .message-notification-panel { width: min(260px, calc(100vw - 24px)); } }
</style>
