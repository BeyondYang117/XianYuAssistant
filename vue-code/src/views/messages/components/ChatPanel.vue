...
const keydown = (event: KeyboardEvent) => { if (event.key === 'Enter' && !event.shiftKey && !event.isComposing) { event.preventDefault(); send() } }
    ...
    <div v-else ref="listRef" class="chat-panel__messages" @scroll="handleScroll">
      ...
      <template v-else-if="messages.length">
        <div v-for="message in messages" :key="message.id" class="chat-message" :class="{ 'chat-message--mine': isMine(message), 'chat-message--system': !isUser(message) && !isMine(message) }">
          ...
          <template v-else>
            <div class="chat-message__meta">{{ isUser(message) ? (message.senderUserName || peerName) : '我' }} · {{ time(message.messageTime) }}</div>
